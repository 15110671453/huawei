            ...
            public void onResult(int retCode, GameUserData userData) {
                if (retCode == HMSAgent.AgentResultCode.HMSAGENT_SUCCESS && userData != null) {
                    ...
                    if (userData.getIsAuth() == 1) {
                        ...
                        // 下面工具方法仅为了展0.示验签请求的逻辑，实际实现应该放在开发者服务端。| The following tool method is intended only to show the logic of the verification request, and the actual implementation should be on the developer server.
